...
void flMethod_slow()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,2500,200,0,210);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,2100,2100); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
void flMethod_slowest()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		...
		speedRec_Init(&glmotorSpeed,1700,1700); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
void flMethod_UpPlatform(void)
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		...
		speedRec_Init(&glmotorSpeed,2250,2250); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
void flMethod_DownPlatform(void)
{
	...
	{
		PID_Init(&glsensorPID,20,2000,200,0,200);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,1600,1600); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
void flMethod_quick()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		...
		speedRec_Init(&glmotorSpeed,4500,5000); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
void flMethod_quickest()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,5500,250,0,3000);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,6500,6440); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
...
void flMethod_16_44()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,18,2500,550,0,500);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,1500,1500); 																				//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value&0x3F0);                                    //分级
	gldSpeed=positionPIDCalc(&glsensorPID,rank);   //速度位置式PID输出
	positionSpeedOut(glmotorSpeed.leftSpeed,glmotorSpeed.rightSpeed,gldSpeed);			//位置式改变电机速度
...
void flMethod_upPeak()
{
	...
	{
   	PID_Init(&glsensorPID,20,2500,300,0,200);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,2250,2240); 																			//对初始速度进行设定
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value);                                    //分级
...
//		PID_Init(&glsensorPID,20,3000,400,0,100);																					//对速度PID的参数进行初始化设置
...
void flMethod_left()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,3500,100,0,0);																					//对速度PID的参数进行初始化设置
...
*/
void flMethod_left_45()
{
	int rank;
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,3500,100,0,0);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,1775,3000); 																				//对初始速度进行设定2200+200
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
	rank=sensorDigitalRank(glsensor_dig_value & 0xF00);                                    //分级
...
void flMethod_right()
{
	...
	if(findLineFlag == 0)//保证每个路段初始化一次
	{
		PID_Init(&glsensorPID,20,3500,160,0,0);																					//对速度PID的参数进行初始化设置
		speedRec_Init(&glmotorSpeed,3500,1000); 																				//对初始速度进行设定2200+200
		findLineFlag=1;
	}
	glsensor_dig_value = sensorAD(glsensor_ad_value,basic_sensorThreshold);  				//与阈值比较后将模拟量转化成数字量
