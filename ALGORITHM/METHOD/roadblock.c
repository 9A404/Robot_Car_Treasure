...
u8 BlockHandleMethod_Step (){
	...
	if(flag==0){
		...
		glHello_control.linkInform.findLineWays = FL_Step;// FL_UpPlatform
		findLineFlag = 0;
		Time3(START); //打开定时器
		gl_time=0;
		flag=2;
	}
//	else if(2==flag && gl_time>30){               //0==PES_Platform
//		Time3(STOP); //关闭定时器
//		gl_time = 0;
//		
////		speedAdjustment(0,0);
